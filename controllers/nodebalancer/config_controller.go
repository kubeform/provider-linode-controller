...
// +kubebuilder:rbac:groups=nodebalancer.linode.kubeform.com,resources=configs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nodebalancer.linode.kubeform.com,resources=configs/status,verbs=get;update;patch
...
func (r *ConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nodebalancerv1alpha1.Config{}); err != nil {
			klog.Error(err, "unable to set up auditor", nodebalancerv1alpha1.Config{}.APIVersion, nodebalancerv1alpha1.Config{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nodebalancerv1alpha1.Config{}).
...
func (r *ConfigReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Config")
			return nil
		}
		...
		resourceList := &nodebalancerv1alpha1.ConfigList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Config")
			return nil
		}
