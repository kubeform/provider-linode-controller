...
// +kubebuilder:rbac:groups=nodebalancer.linode.kubeform.com,resources=nodes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nodebalancer.linode.kubeform.com,resources=nodes/status,verbs=get;update;patch
...
func (r *NodeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nodebalancerv1alpha1.Node{}); err != nil {
			klog.Error(err, "unable to set up auditor", nodebalancerv1alpha1.Node{}.APIVersion, nodebalancerv1alpha1.Node{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nodebalancerv1alpha1.Node{}).
...
func (r *NodeReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Node")
			return nil
		}
		...
		resourceList := &nodebalancerv1alpha1.NodeList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Node")
			return nil
		}

		for _, res := range resourceList.Items {
			oka := make(map[string]interface{})
			byt, _ := json.Marshal(res)
			err := json.Unmarshal(byt, &oka)
			tmp, found, err := unstructured.NestedString(oka, "spec", "secretRef", "name")
