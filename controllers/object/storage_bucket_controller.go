...
// +kubebuilder:rbac:groups=object.linode.kubeform.com,resources=storagebuckets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=object.linode.kubeform.com,resources=storagebuckets/status,verbs=get;update;patch

func (r *StorageBucketReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagebucket", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageBucket")
...
func (r *StorageBucketReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&objectv1alpha1.StorageBucket{}).
...
func (r *StorageBucketReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for StorageBucket")
			return nil
		}
		...
		resourceList := &objectv1alpha1.StorageBucketList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list StorageBucket")
			return nil
		}
